...
export default function Area() {
    ...
    const getData = () => {
        if (!loading) {
            setLoading(true);
            get(`location/title?path=${params.title}`)
                .then(res => {
                    setLoading(false);
    ...
    return (
        <>
            ...
            {data && data?.property?.length ?
                <section className='onward-workspaces-offerings top bottom padding-left-right'>
                    <div className='workspaces-slider'>
                        <div className='heading-title'>
                            <h2 className='heading text-center workspaces'>{data?.proHaed ? data?.proHaed : "Onward Workspaces Offerings"}</h2>
            ...
            {/* <section className='onward-workspaces-offerings top-0 bottom padding-left-right'>
                <div className='workspaces-slider'>
                    <div className='heading-title'>
                        <h2 className='heading text-center padding_bottom workspaces'>Workspaces Available in {data?.title}</h2>
                    </div>
                    <div className='slider-section'>
                        <Swiper
                            slidesPerView={1}
                        ...
                        >
                            {data?.property?.map((item) => (
                                <SwiperSlide>
                                    <Link to={`/property/${parseTitle(item?.title)}`} className='text-decoration-none'>
                                        <div className='slider-item'>
                ...
                <section className='onward-workspaces-offerings top-0 bottom padding-left-right near_by_area'>
                    <div className='workspaces-slider'>
                        <div className='heading-title padding-left-right'>
                            <h3 className='heading text-center workspaces'>{data?.locHead ? data?.locHead : "Near By Area"}</h3>
                            <p className='paragraph' style={{ whiteSpace: 'pre-line' }}>​{data?.locPara}</p>
                        </div>
                        {
                            data?.nearBy?.length <= 3 ?
                                <div className='normal-design-box'>
                                    <div className='row justify-content-center'>
                                        {data?.nearBy?.map((item) => (
                                            <div className='col-lg-4 col-md-6'>
                                                <Link to={`/${parseTitle(data?.city[0]?.path)}/${parseTitle(item?.path)}`}>
                                                    <div className='nearby-gallery'>
                                                        <img src={item?.image ? item?.image : require('../../assets/images/rectangle_166.png')} alt='rectangle_166' />
                                                        <div className='n-content-box-s'>
                                                            <div className='n-content-box'>
                                                                <h4>{data?.city?.length ? data?.city[0]?.title : ""}</h4>
                                ...
                                <div className='explore-section slider-section mt-0'>
                                    ...
                                    >
                                        {data?.nearBy?.map((item) => (
                                            <SwiperSlide>
                                                <Link to={`/${(data?.city[0]?.path)}/${parseTitle(item?.title)}`}>
                                                    <div className='nearby-gallery'>
                                                        <img src={item?.image ? item?.image : require('../../assets/images/rectangle_166.png')} alt='rectangle_166' />
                                                        <div className='n-content-box-s'>
            ...
            {data && data?.testi?.length ?
                <section className='onward-workspaces-offerings top-0 bottom padding-left-right'>
                    <div className='workspaces-slider'>
                        <div className='heading-title'>
                            <h4 className='heading text-center workspaces'>{data?.testiHead ? data?.testiHead : "Onward Workspaces Offerings"}</h4>
