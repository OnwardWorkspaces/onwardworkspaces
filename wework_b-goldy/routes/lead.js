...
router.get("/", authMiddleware, async (req, res) => {
    try {
        let users = await leadController.getDataAdmin(req);
...
//     try {
//         let user = await leadController.getUserById(req);
...
router.post("/", async (req, res) => {
    try {
        let users = await leadController.addData(req);
        let code = users.statusCode;
...
router.put("/", authMiddleware, async (req, res) => {
    try {
        let users = await leadController.updateData(req);
        let code = users.statusCode;
        res.status(code).send(users);
    } catch (error) {
        res.status(500).send({
...
router.delete("/", authMiddleware, async (req, res) => {
    try {
        let users = await leadController.removeData(req);
        let code = users.statusCode;
        // delete users.statusCode;
        res.status(code).send(users);
    } catch (error) {
