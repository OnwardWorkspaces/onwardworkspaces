...
subsController = require("./../controllers/subsController");
...
router.get("/", authMiddleware, async (req, res) => {
    try {
        let users = await subsController.getDataAdmin(req);
...
//     try {
//         let user = await subsController.getUserById(req);
...
router.post("/", async (req, res) => {
    try {
        let users = await subsController.addData(req);
        let code = users.statusCode;
...
router.put("/", authMiddleware, async (req, res) => {
    try {
        let users = await subsController.updateData(req);
        let code = users.statusCode;
        res.status(code).send(users);
    } catch (error) {
        res.status(500).send({
...
router.delete("/", authMiddleware, async (req, res) => {
    try {
        let users = await subsController.removeData(req);
        let code = users.statusCode;
        // delete users.statusCode;
        res.status(code).send(users);
    } catch (error) {
