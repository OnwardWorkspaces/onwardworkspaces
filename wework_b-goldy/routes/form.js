...
router.post("/image_upload", async (req, res) => {
    try {
        let image = await formController.imageUpload(req);
...
router.get("/", authMiddleware, async (req, res) => {
    try {
        let users = await formController.getDataAdmin(req);
...
router.get("/list", authMiddleware, async (req, res) => {
    try {
        let users = await formController.getData(req);
        let code = users.statusCode;
        res.status(code).send(users);
    } catch (error) {
        console.log('error while getting users', error);
...
//     try {
//         let user = await formController.getUserById(req);
...
router.post("/", authMiddleware, async (req, res) => {
    try {
        let users = await formController.addData(req);
        let code = users.statusCode;
        // delete users.statusCode;
        res.status(code).send(users);
    } catch (error) {
...
router.put("/", authMiddleware, async (req, res) => {
    try {
        let users = await formController.updateData(req);
        let code = users.statusCode;
        res.status(code).send(users);
    } catch (error) {
        res.status(500).send({
...
router.delete("/", authMiddleware, async (req, res) => {
    try {
        let users = await formController.removeData(req);
        let code = users.statusCode;
        // delete users.statusCode;
        res.status(code).send(users);
    } catch (error) {
