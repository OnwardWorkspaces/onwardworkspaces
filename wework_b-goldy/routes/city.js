...
router.get("/", authMiddleware, async (req, res) => {
    try {
        let users = await cityController.getDataAdmin(req);
...
router.get("/list", authMiddleware, async (req, res) => {
    try {
        let users = await cityController.getData(req);
        let code = users.statusCode;
        res.status(code).send(users);
    } catch (error) {
        console.log('error while getting users', error);
...
router.get("/drop", async (req, res) => {
    try {
        let users = await cityController.getDrop(req);
        let code = users.statusCode;
        res.status(code).send(users);
    } catch (error) {
        console.log('error while getting users', error);
...
router.get("/title", async (req, res) => {
    try {
        let users = await cityController.getCityByTitle(req);
        if (users?.data?.form?.[0]?.field) {
            users.data.form[0].field = users.data.form[0].field.filter(
...
//     try {
//         let user = await cityController.getUserById(req);
...
router.post("/", authMiddleware, async (req, res) => {
    try {
        let users = await cityController.addData(req);
        let code = users.statusCode;
...
router.put("/", authMiddleware, async (req, res) => {
    try {
        let users = await cityController.updateData(req);
        let code = users.statusCode;
        res.status(code).send(users);
    } catch (error) {
        res.status(500).send({
...
router.delete("/", authMiddleware, async (req, res) => {
    try {
        let users = await cityController.removeData(req);
        let code = users.statusCode;
        // delete users.statusCode;
        res.status(code).send(users);
    } catch (error) {
...
//     try {
//         let users = await cityController.banUser(req);
//         let code = users.statusCode;
//         // delete users.statusCode;
//         res.status(code).send(users);
//     } catch (error) {
...
router.post("/image_upload", async(req, res)=>{
    try{
        let image = await cityController.imageUpload(req);
