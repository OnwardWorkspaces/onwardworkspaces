...
router.post("/image_upload", async (req, res) => {
    try {
        let image = await categoryController.imageUpload(req);
...
router.get("/", authMiddleware, async (req, res) => {
    try {
        let users = await categoryController.getDataAdmin(req);
...
router.get("/list", async (req, res) => {
    try {
        let users = await categoryController.getData(req);
        const orderMap = {
            "Managed Office": 1,
            "Private Suites": 2,
            "Private Cabins": 3,
        ...
        // Apply sorting before sending response
        if (users?.data?.length) {
            users.data.sort((a, b) => {
                return (orderMap[a.title] || 999) - (orderMap[b.title] || 999);
            });
        }
        let code = users.statusCode;
        res.status(code).send(users);
...
router.get("/title", async (req, res) => {
    try {
        let users = await categoryController.getDataByTitle(req);
        let code = users.statusCode;
        res.status(code).send(users);
    } catch (error) {
        console.log('error while getting users', error);
...
//     try {
//         let user = await categoryController.getUserById(req);
...
router.post("/", authMiddleware, async (req, res) => {
    try {
        let users = await categoryController.addData(req);
        let code = users.statusCode;
        // delete users.statusCode;
        res.status(code).send(users);
    } catch (error) {
...
router.put("/", authMiddleware, async (req, res) => {
    try {
        let users = await categoryController.updateData(req);
        let code = users.statusCode;
        res.status(code).send(users);
    } catch (error) {
        res.status(500).send({
...
router.delete("/", authMiddleware, async (req, res) => {
    try {
        let users = await categoryController.removeData(req);
        let code = users.statusCode;
        // delete users.statusCode;
        res.status(code).send(users);
    } catch (error) {
