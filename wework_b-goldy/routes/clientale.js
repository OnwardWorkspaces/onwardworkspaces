...
clientaleController = require("../controllers/clientaleController");
...
router.post("/image_upload", async (req, res) => {
    try {
        let image = await clientaleController.imageUpload(req);
...
router.get("/", authMiddleware, async (req, res) => {
    try {
        let users = await clientaleController.getDataAdmin(req);
...
router.get("/list", authMiddleware, async (req, res) => {
    try {
        let users = await clientaleController.getData(req);
        let code = users.statusCode;
        res.status(code).send(users);
    } catch (error) {
        console.log('error while getting users', error);
...
//     try {
//         let user = await clientaleController.getUserById(req);
...
router.post("/", authMiddleware, async (req, res) => {
    try {
        let users = await clientaleController.addData(req);
        let code = users.statusCode;
        // delete users.statusCode;
        res.status(code).send(users);
    } catch (error) {
...
router.put("/", authMiddleware, async (req, res) => {
    try {
        let users = await clientaleController.updateData(req);
        let code = users.statusCode;
        res.status(code).send(users);
    } catch (error) {
        res.status(500).send({
...
router.delete("/", authMiddleware, async (req, res) => {
    try {
        let users = await clientaleController.removeData(req);
        let code = users.statusCode;
        // delete users.statusCode;
        res.status(code).send(users);
    } catch (error) {
