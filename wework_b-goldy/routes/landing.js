...
router.post("/image_upload", async (req, res) => {
    try {
        let image = await landingController.imageUpload(req);
...
router.get("/", authMiddleware, async (req, res) => {
    try {
        let users = await landingController.getDataAdmin(req);
...
router.get("/title", async (req, res) => {
    try {
        let users = await landingController.getDataByTitle(req);
        let code = users.statusCode;
        res.status(code).send(users);
    } catch (error) {
        console.log('error while getting users', error);
...
//     try {
//         let user = await landingController.getUserById(req);
...
router.post("/", authMiddleware, async (req, res) => {
    try {
        let users = await landingController.addData(req);
        let code = users.statusCode;
        // delete users.statusCode;
        res.status(code).send(users);
    } catch (error) {
...
router.put("/", authMiddleware, async (req, res) => {
    try {
        let users = await landingController.updateData(req);
        let code = users.statusCode;
        res.status(code).send(users);
    } catch (error) {
        res.status(500).send({
...
router.delete("/", authMiddleware, async (req, res) => {
    try {
        let users = await landingController.removeData(req);
        let code = users.statusCode;
        // delete users.statusCode;
        res.status(code).send(users);
    } catch (error) {
