 ...
 *                     items: 
 *                       type: object
 *                       example: { "_id": "644b4f4cce392c2bd8b6acca", "banner": "https://eeasy.s3.ap-south-1.amazonaws.com/aahilya/case/1682657097531.JPEG", "desc": "<p>zxcvbnm</p>\n", "isActive": true, "isDeleted": false, "name": "India 2", "createdAt": "2023-04-28T04:45:00.085Z", "updatedAt": "21 days ago", "__v": 0 }
 *                  message:
 *                     type: string
 *                     example: SUCCESS
 *                  statusCode:
...
router.get("/list", async (req, res) => {
    try {
        let experiences = await caseController.getData(req);
...
router.get("/drop", async (req, res) => {
    try {
        let experiences = await caseController.getDrop(req);
        let code = experiences.statusCode;
        res.status(code).send(experiences);
    } catch (error) {
        console.log('error while getting experiences', error);
...
router.get("/title", async (req, res) => {
    try {
        let experience = await caseController.getDataByTitle(req);
...
router.post("/add", authMiddleware, async (req, res) => {
    try {
        let experiences = await caseController.addData(req);
        let code = experiences.statusCode;
...
router.put("/update", authMiddleware, async (req, res) => {
    try {
        let experiences = await caseController.updateData(req);
        let code = experiences.statusCode;
        res.status(code).send(experiences);
    } catch (error) {
        res.status(500).send({
...
router.post("/delete", authMiddleware, async (req, res) => {
    try {
        let experiences = await caseController.removeData(req);
        let code = experiences.statusCode;
        // delete experiences.statusCode;
        res.status(code).send(experiences);
    } catch (error) {
...
router.post("/image_upload", async (req, res) => {
    try {
        let image = await caseController.imageUpload(req);
