...
import AddForm from "./AddClientale";
...
function Clientale() {
    ...
    const getData = () => {
        if (!loading) {
            setLoading(true);
            get("clientale", { token: user?.token })
                .then(res => {
                    setLoading(false);
    ...
    const handleValidSubmit = async (e, v) => {
        if (!loading) {
            ...
            if (file?.substring(0, 4) == 'blob') {
                ...
                const uploadRes = await upload("image_upload", form);
                ...
                {isAdd ?
                    ...
                    <Card>
                        <Row className='mb-4'>
                            <Col md={11}>
                                <CardBody>
                                    <CardTitle><b>Clientale</b></CardTitle>
                                    <CardSubtitle>Clientales you have added before</CardSubtitle>
                                </CardBody>
                            </Col>
                            {
