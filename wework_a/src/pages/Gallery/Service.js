...
function WorkCategory() {
    ...
    useEffect(() => {
        setColumns([
            ...
            {
                name: 'Highlights',
                selector: row => row?.highlight?.length,
                maxWidth: '110px'
            },
            {
                cell: (row) => <>
            ...
            },
            {
                ...
                style={{ backgroundColor: Utils.themeColor, marginRight: 10, padding: 5, paddingLeft: 8, paddingRight: 8, border: 'none' }}><span className="mdi mdi-eye" style={{ fontSize: 20 }}></span></Button> */}
                    {
                        checkPermission('workspaceService', 'update') &&
                    ...
                    {
                        checkPermission('workspaceService', 'delete') &&
    ...
    const getData = () => {
        if (!loading) {
            setLoading(true);
            get("workspace", { token: user?.token })
                .then(res => {
                    setLoading(false);
    ...
    const getCats = () => {
        get("workspace/category/list", { token: user?.token })
            .then(res => {
                if (res?.statusCode == 200) {
                    setCats(res?.data);
                }
                ...
                <div className="main main-app p-3 p-lg-4">
                    <Card>
                        <Row className='mb-4'>
                            <Col md={11}>
                                <CardBody>
                                    <CardTitle><b>Work-Space Services</b></CardTitle>
                                    <CardSubtitle>Services you have added before</CardSubtitle>
                                </CardBody>
                            </Col>
                            {
                                checkPermission('workspaceService', 'write') &&
                                <Col md={1}>
