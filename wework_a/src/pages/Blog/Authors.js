...
function BlogAuthor() {
    ...
    const getData = () => {
        if (!loading) {
            setLoading(true);
            get("blog/author", { token: user?.token })
                .then(res => {
                    setLoading(false);
    ...
    const handleValidSubmit = async (e, v) => {
        if (!loading) {
            ...
            if (currentData?._id) {
                body = { ...body, authorId: currentData?._id }
                put("blog/author", body)
                    .then(res => {
                        setLoading(false);
                        if (res?.statusCode == 200) {
                            setIsAdd(false);
        ...
        <React.Fragment>
            ...
            <div className="main main-app p-3 p-lg-4">
                <Card>
                    <Row className='mb-4'>
                        <Col md={11}>
                            <CardBody>
                                <CardTitle><b>Blog Author</b></CardTitle>
                                <CardSubtitle>Authors you have added before</CardSubtitle>
                            </CardBody>
                        </Col>
            ...
            <Modal isOpen={isAdd} centered>
                <ModalBody>
                    <Card>
                        <CardBody>
                            <CardTitle className='mb-4'><b>{currentData?._id ? 'Update Author' : 'Add New Author'}</b></CardTitle>
