...
const Table = (props) => {
  ...
  const getMetas = () => {
    ...
    //     console.log('error while getting properties on blogs', error);
    //   });
    get("blog/list", { token: user?.token })
      .then(json => {
  ...
  useEffect(() => {
    setColumns([
      ...
      {
        name: 'Category',
        selector: row => row.categories[0]?.title,
        maxWidth: '100px'
      },
      {
        name: 'Author',
        selector: row => row.authors[0]?.title,
        maxWidth: '100px'
      },
      {
        name: 'Last Updated',
      ...
      },
      {
        cell: (row) => <>
          {user?.role == 'Admin' &&
            <Button
              title={"View"}
              style={{ backgroundColor: Utils.themeColor, marginRight: 5, padding: 5, paddingLeft: 8, paddingRight: 8 }}
              onClick={()=>openBlog(row)}><span className="ri-eye-fill" style={{ color: '#fff', fontSize: 20 }}></span>
            </Button>
          }
          {canUpdate &&
  ...
  const handleUpdateEdit = (proj) => {
    ...
    navigate("/blog/add-blog", { state: proj })
  }
  ...
  const openBlog = (item) => {
    window.open(`http://3.108.12.59:3000/blog/${parseName(item?.title)}`);
    // window.open(`http://localhost:3000/blog/${parseName(item?.title)}`);
  }
            ...
            <Row>
              ...
              <Col md={1}>
                <div style={{ display: 'flex', alignSelf: 'flex-end' }}>
                  {canAdd &&
                    <Button style={{ backgroundColor: Utils.themeColor }} onClick={() => navigate("/blog/add-blog")}>
                      Add
                    </Button>
                  }
                </div>
